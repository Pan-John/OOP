...
void MY_SYSTEM_APP::draw_SineCosineFunction( ) const
{
	...
	double x0, x1;
	mSineCosineFunction.getXInterval( x0, x1 );
	...
	for ( int i = 0; i < numSamples; ++i ) {
		...
		y = mSineCosineFunction.getFunctionValue( x );
		glVertex3f( x, 0.0, y );
	}
	glEnd( );
}
...
void MY_SYSTEM_APP::draw_CubicFunction( ) const
{
	...
	double x0, x1;
	mCubicFunction.getXInterval( x0, x1 );
	glBegin(GL_LINE_STRIP );
	...
	for ( int i = 0; i < numSamples; ++i ) {
		...
		float x;
		x = x0 + (i/((double) numSamples -1))*(x1 - x0);
		y = mCubicFunction.getFunctionValue( x );
		glVertex3f( x, 0.0, y );
	}
	glEnd( );
	...
	//glColor3f(0.0, 0.7, 0.0 );//i change color here to see more clear
    glBegin(GL_LINE_STRIP );
