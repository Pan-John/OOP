...
double MONTE_CARLO_SYSTEM::getValue(double x) const
{
    ...
    case FUNCTION_EXPONENTIAL_SINE:
        y = 5.0*pow(2.718281828, -x*x/4.0)*cos(x);/////////sin(x)->cos(x) ; -x ->-x*x
        break;
    case FUNCTION_COSINE:
        y = cos(4 * x) * sin(6 * x) - x / (1 + x * x);
        break;
    case FUNCTION_SINE_COSINE:
        y = -3 * cos(x * x) + sin(6 * x) / (abs(x - 4) + 0.01);
        break;
...
bool MONTE_CARLO_SYSTEM::isInsideArea( double x, double y ) const
{
    ...
    else if ( realY <= 0.0 && y <= 0.0 && y >= realY ) return true;
    //
    // fill/modify your own stuff ////check
    //
    return false;
}