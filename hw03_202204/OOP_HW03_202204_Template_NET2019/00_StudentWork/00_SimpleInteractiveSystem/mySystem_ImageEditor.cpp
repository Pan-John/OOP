...
IMAGE_SYSTEM::IMAGE_SYSTEM()
{
    ...
    m_ImageHierarchy->computeDrawPosition(wImage, hImage);
    //
}
...
{
    node->flg_explored = false;//from high to low reset
    for (int i = 0; i < 4; ++i) {
        if (node->children[i]) {
...
//
void IMAGE_SYSTEM::computeNodeColor(IMAGE_NODE *node)//done!
{
    IMAGE_NODE* n = node;
    computeAverageColor(
        n->start_x,
...
//
void IMAGE_SYSTEM::exploreImageNode(
    IMAGE_NODE *node,
    double x,
    double y)
{
    ...
    if (node->flg_explored) {
        ...
        children[0]->start_x = node->start_x;//+node->w/2;
        children[0]->start_y = node->start_y;// -node->h / 2;//+node->h/2;
        ...
        children[2]->start_y = node->start_y+node->h / 2;
        ...
        children[3]->start_y = node->start_y+node->h/2;
